...
public class UIButtonScale : TICoroutineMonoBehaviour
{
	public Transform tweenTarget;
	public Vector3 hover = new Vector3(1.1f, 1.1f, 1.1f);
	public Vector3 pressed = new Vector3(1.05f, 1.05f, 1.05f);
	public float duration = 0.2f;
	public Transform[] otherTweenTargets;
	...
	void OnPress (bool isPressed)
	{
		if (enabled)
		{
			...
			for (int i = 0; i < otherTweenTargets.Length; i++)
			{
				TweenScale.Begin(otherTweenTargets[i].gameObject, duration, isPressed ? Vector3.Scale(mScale, pressed) :
					(UICamera.IsHighlighted(gameObject) ? Vector3.Scale(mScale, hover) : mScale)).method = UITweener.Method.EaseInOut;
			}
		}
	}
	...
	void OnHover (bool isOver)
	{
		if (enabled)
		{
			...
			for (int i = 0; i < otherTweenTargets.Length; i++)
			{
				TweenScale.Begin(otherTweenTargets[i].gameObject, duration, isOver ? Vector3.Scale(mScale, hover) : mScale).method = UITweener.Method.EaseInOut;
			}			
			mHighlighted = isOver;
		}
	}
