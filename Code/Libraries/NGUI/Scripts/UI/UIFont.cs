...
{
	public static bool Intersection( this Rect rect, Rect other, ref Rect intersection )
	{
		float l = Mathf.Max( rect.xMin, other.xMin );
		float t = Mathf.Max( rect.yMin, other.yMin );
		float r = Mathf.Min( rect.xMax, other.xMax );
		float b = Mathf.Min( rect.yMax, other.yMax );
		
		if( l < r && t < b )
		{
			intersection = new Rect( l, t, r - l, b - t );
			return true;
		}
		return false;
	}
	
	public static bool IntersectionInvY( this Rect rect, Rect other, ref Rect intersection )
	{
		float l = Mathf.Min( rect.xMin, other.xMin );
		float t = Mathf.Min( rect.yMin, other.yMin );
		float r = Mathf.Max( rect.xMax, other.xMax );
		float b = Mathf.Max( rect.yMax, other.yMax );
...
public class UIFont : TICoroutineMonoBehaviour
{
	...
	// Added by Zynga/Todd.
	public static string[] breakingChars = new string[] { " ", "-" };
	...
	public Vector2 CalculatePrintedSize (string text, bool encoding, SymbolStyle symbolStyle, int lineSpacing)
	{
		if (mReplacement != null) return mReplacement.CalculatePrintedSize(text, encoding, symbolStyle, lineSpacing);

		Vector2 v = Vector2.zero;
		bool dynamic = isDynamic;
		
		lineSpacing = (int)(lineSpacing / pixelSize);
...
#endif
		{
			...
			int prev = 0;
			int fs = size;
			int lineHeight = (fs + mSpacingY + lineSpacing);	// Zynga - Todd added lineSpacing.
			...
			v.y = scale * (y + lineHeight - lineSpacing);	// Zynga - Todd: subtract one lineSpacing since we don't want to include spacing for the last line.
		}
		return v;
	}
	...
	public bool WrapText (string text, out string finalText, float width, float height, int lines, bool encoding, SymbolStyle symbolStyle)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			// Zynga - Word-wrap on hyphens - MRH
			if (isBreakingChar(ch) && !isBreakingChar((char)previousChar) && start < offset)
			{
			...
			if (encoding && ch == '[')
			{
				if (offset + 2 < textLength)
				{
					...
					{
						NGUITools.ParseColor (text, offset + 1, out outColor);
						if (NGUITools.EncodeColor(outColor) == text.Substring(offset + 1, 6).ToUpper())
						{
							offset += 7;
							continue;
						}
			...
			if (remainingWidth < 0)
			{
				...
				if (lineIsEmpty || !multiline || lineCount == maxLineCount)
				{
					...
					// Zynga - Todd
					// Don't break a word in the middle, even if it causes the word to spill outside the label area.
					// It still looks better to allow the text to spill over than to chop a word in half for wrapping.
					// Forward the offset until we find the next break, or the end of the text, and use the whole word.
					while (true)
					{
						offset++;
						...
						{
							char futureCh = text[offset];
							if (isBreakingChar(futureCh))
							{
...
//					sb.Append(text.Substring(start, Mathf.Max(0, offset - start)));
	...
	// Zynga - can it word-wrap on this character? - MRH
	private bool isBreakingChar(char ch)
	{
		return (System.Array.IndexOf(breakingChars, ch.ToString()) > -1);
	}
	...
	public void Print (string text, Color32 color, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols,
		bool encoding, SymbolStyle symbolStyle, Alignment alignment, int lineWidth, bool premultiply, int lineSpacing)
	{
		if (mReplacement != null)
		{
			mReplacement.Print(text, color, verts, uvs, cols, encoding, symbolStyle, alignment, lineWidth, premultiply, lineSpacing);
		}
		else if (text != null)
		{
			if (!isValid)
			...
			int y = 0;
			int prev = 0;
			int lineHeight = (fs + mSpacingY + lineSpacing);	// Zynga - Todd added lineSpacing variable.
	...
	private List<GradientStep> mGradientSteps = new List<GradientStep>();
	
	public void PrintGradient (string text, Color startColor, Color endColor, List<GradientStep> steps, float gradientScale, float gradientOffset,
		BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols,
		bool encoding, SymbolStyle symbolStyle, Alignment alignment, int lineWidth, bool premultiply, int lineSpacing)
	{
		if (mReplacement != null)
		{
			mReplacement.PrintGradient(text, startColor, endColor, steps, gradientScale, gradientOffset, verts, uvs, cols, encoding, symbolStyle, alignment, lineWidth, premultiply, lineSpacing);
		}
		else if (text != null)
		{
			if (!isValid)
			...
			mColors.Clear();
			mGradientSteps.Clear();
			
			mGradientSteps.Add( new GradientStep( 0, startColor ) );
			
			if( steps != null && steps.Count > 0 )
			{
				mGradientSteps.AddRange( steps );
			}
			
			mGradientSteps.Add( new GradientStep( 1, endColor ) );
			...
			int prev = 0;
			int lineHeight = (fs + mSpacingY + lineSpacing);	// Zynga - Todd added lineSpacing variable.
			int lineHeightGradient = (fs + mSpacingY);			// Zynga - Todd added. We can't include lineSpacing in the line height used for
																// determining the gradient area, or the text gets cut off with negative spacing.
			Vector3 v0 = Vector3.zero, v1 = Vector3.zero;
			Vector2 u0 = Vector2.zero, u1 = Vector2.zero;
			float invX = uvRect.width / mFont.texWidth;
			float invY = mUVRect.height / mFont.texHeight;
			...
			Vector2 gv0, gv1, gu0, gu1;
			Vector2 dv, gdv;
			Vector2 du, gdu;
			
			Rect vertArea, gradientArea, uvArea, stepArea, intersection;
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (!dynamic)
				{
					...
					if (symbol == null)
					{
						...
						vertArea = new Rect( v0.x, v0.y, dv.x, dv.y );
						gradientArea = new Rect( v0.x, -invSize.y * y, invSize.x * glyph.width, -invSize.y * lineHeightGradient);
						
						c0 = startColor;
						
						// go through each gradient step and calculate vertex posision, UVs, color, etc.
						for( int gl = 0; gl < mGradientSteps.Count - 1; gl++ )
						{
							g0 = mGradientSteps[ gl ];
							g1 = mGradientSteps[ gl + 1 ];
							...
							float loc0 = g0.location;
							float loc1 = g1.location;
							
							// calculate gradient step area
							gv0.y = gradientArea.y + gradientArea.height * loc0;
							gv1.y = gradientArea.y + gradientArea.height * loc1;
							gdv = gv1 - gv0;

							stepArea = new Rect( gv0.x, gv0.y, gdv.x, gdv.y );
							intersection = stepArea;
							
							c1 = g1.color;
							
							// get the intersection area of the gradient step and vertex areas
							if( stepArea.IntersectionInvY( vertArea, ref intersection ) )
							{
								verts.Add(new Vector3(intersection.xMax, intersection.yMin));
								verts.Add(new Vector3(intersection.xMax, intersection.yMax));
								verts.Add(new Vector3(intersection.xMin, intersection.yMax));
								verts.Add(new Vector3(intersection.xMin, intersection.yMin));
								
								// get the percentage in the Y direction the vertex area is in respecto to the intersection box
								float p0 = ( v0.y - intersection.yMin ) / -vertArea.height;
								float p1 = ( v0.y - intersection.yMax ) / -vertArea.height;

								// calculate UVs based on percent down the vertex area the intersection is
								gu0.y = u0.y + du.y * p0;
								gu1.y = u0.y + du.y * p1;

								// calculate the UV rectangle
								gdu = gu1 - gu0;
								uvArea = new Rect( gu0.x, gu0.y, gdu.x, gdu.y );

								uvs.Add(new Vector2(uvArea.xMax, uvArea.yMin));
								uvs.Add(new Vector2(uvArea.xMax, uvArea.yMax));
								uvs.Add(new Vector2(uvArea.xMin, uvArea.yMax));
								uvs.Add(new Vector2(uvArea.xMin, uvArea.yMin));
								
								// calculate percentage of the grandiant area the vert is in to properly lerp color values of missing verticies
								//  gp0 = 0: top vert is inside step area
								//  gp0 > 0: top vert is outside step area
								//  ( gp1 + loc0 ) / loc1 = 1: bottom vert inside step area
								//  ( gp1 + loc0 ) / loc1 < 1: bottom vert outside step area
								float gp0 = ( gv0.y - intersection.yMin ) / -gradientArea.height;
								float gp1 = ( gv0.y - intersection.yMax ) / -gradientArea.height;
								gp1 += loc0;
								
								c0 = Color.Lerp( c0, c1, gp0 );
								c1 = Color.Lerp( c0, c1, gp1 / loc1 );
								// Use the normal color's alpha value for every color in the gradient,
								// since that can be animated for fading effects.
								c0.a = startColor.a;
								c1.a = startColor.a;
