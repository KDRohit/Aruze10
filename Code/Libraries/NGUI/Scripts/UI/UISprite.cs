...
public class UISprite : UIWidget
{
	public enum Type
	{
		...
		Tiled,
		Filled,
		SlicedInverted,	// Added by Zynga / Todd. Similar to Sliced, but rather than keeping the corners fixed in size, it keeps the center fixed
	}
	...
	public override Material material { get { mMatCached = (!System.Object.ReferenceEquals(mAtlas,null)) ? mAtlas.spriteMaterial : null;return mMatCached; } }
	...
	public UIAtlas atlas
	{
		...
		set
		{
			if (mAtlas != value)
			{
				...
				// Update the material
				mMatCached = (!System.Object.ReferenceEquals(mAtlas,null)) ? mAtlas.spriteMaterial : null;
	...
	public UIAtlas.Sprite GetAtlasSprite ()
	{
		...
		if (mSprite == null && mAtlas != null)
		{
			...
			if (!System.Object.ReferenceEquals(mAtlas,null))
			{
				mMatCached = mAtlas.spriteMaterial;
				UpdateUVs(true);
			}
		}
		return mSprite;
	...
	virtual public void UpdateUVs (bool force)
	{
			...
			))
		{
			...
			if (tex != null)
			{
				...
				pivotOffsetFromAtlas = mSprite.pivotOffset;	// Added by Zynga/Todd.
	...
	public override void MakePixelPerfect ()
	{
		...
		if (t == Type.Sliced || t == Type.SlicedInverted)
		{
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		switch (type)
		{
			...
			case Type.Sliced:
			case Type.SlicedInverted:	// Added by Zynga / Todd
...
#region Various fill functions
	...
	protected void SimpleFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        tempv3.x = 1f;
        tempv3.y = 0f;
        vertsbuffer[size]   = tempv3;//1,0
        tempv3.y = -1f;
        vertsbuffer[size+1] = tempv3;//1,-1
        tempv3.x = 0f;
        vertsbuffer[size+2] = tempv3;//0,-1
        tempv3.y = 0f;
        vertsbuffer[size+3] = tempv3;//0,0

        size = uvs.size;
        uvs.AddCount(4);
        Vector2[] uvsbuffer = uvs.buffer;
        
        tempv2.x = mOuterUV.xMax;
        tempv2.y = mOuterUV.yMax;
		
		uvsbuffer[size] =tempv2;
		tempv2.y = mOuterUV.yMin;
		uvsbuffer[size+1] = tempv2;

		tempv2.x = mOuterUV.xMin;
		uvsbuffer[size+2] = tempv2;
		
		tempv2.y = mOuterUV.yMax;
		uvsbuffer[size+3] = tempv2;
	    ...
	    size = cols.size;
		cols.AddCount(4);
        Color32[] colbuffer = cols.buffer;
        
        colbuffer[size]   = col;
        colbuffer[size+1] = col;
        colbuffer[size+2] = col;
        colbuffer[size+3] = col;
	}
	...
	private static Vector3 tempv3 = new Vector3(0f,0f,0f);
	private static Vector2 tempv2 = new Vector2(0f,0f);
	...
	protected void SlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (tex != null)
		{
			...
			if (type == Type.SlicedInverted)
			{
				// Added by Zynga / Todd.
				// Split the space on the sides in half evenly, regardless of border definitions.
				// The middle stays the same size no matter how large it's scaled.
				float leftRightSides = Mathf.Max(0.0f, sz.x - mInnerUV.size.x) * 0.5f;
				float topBottomSides = Mathf.Max(0.0f, sz.y - mInnerUV.size.y) * 0.5f;
				
				// These values are both the same
				tl = br = new Vector2(leftRightSides / sz.x, topBottomSides / -sz.y);
			}
			else
			{
				tl = new Vector2(borderLeft / sz.x, borderTop / sz.y);
				br = new Vector2(borderRight / sz.x, borderBottom / sz.y);
			}
		...
		bool isSlicedInverted = (type == Type.Sliced) && !mFillCenter;
		...
		verts.AddCount(toAdd);
		uvs.AddCount(toAdd);
		cols.AddCount(toAdd);

		Vector3 [] verts_buffer = verts.buffer;
		Vector2 [] uvs_buffer = uvs.buffer;
		Color32 [] cols_buffer = cols.buffer;
		...
		for (int x = 0; x < 3; ++x)
		{
			...
			bool checkSlicedInverted = isSlicedInverted && (x == 1);

			for (int y = 0; y < 3; ++y)
			{
				if (checkSlicedInverted && y == 1) 
					continue;	// Zynga / Todd: Added type to condition, since it doesn't apply to SlicedInverted.
				...
				verts_buffer[idx] = tempv3;//verts.Add(new Vector3(v[x2].x, v[y].y, 0f));

				tempv3.y = v[y2].y;
				verts_buffer[idx+1] = tempv3;//verts.Add(new Vector3(v[x2].x, v[y2].y, 0f));

				tempv3.x = v[x].x;
				verts_buffer[idx+2] = tempv3;//verts.Add(new Vector3(v[x].x, v[y2].y, 0f));
				
				tempv3.y = v[y].y;
				verts_buffer[idx+3] = tempv3;//verts.Add(new Vector3(v[x].x, v[y2].y, 0f));

				tempv2.x = uv[x2].x;
				tempv2.y = uv[y].y;
				uvs_buffer[idx] = tempv2;

				tempv2.y = uv[y2].y;
				uvs_buffer[idx+1] = tempv2;

				tempv2.x = uv[x].x;
				uvs_buffer[idx+2] = tempv2;

				tempv2.y = uv[y].y;
				uvs_buffer[idx+3] = tempv2;

				cols_buffer[idx] = col;
				cols_buffer[idx+1] = col;
				cols_buffer[idx+2] = col;
				cols_buffer[idx+3] = col;
