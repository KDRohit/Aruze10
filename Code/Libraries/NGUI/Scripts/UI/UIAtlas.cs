...
public class UIAtlas : TICoroutineMonoBehaviour
{
	...
	public class Sprite
	{
		...
		// Added by Zynga/Todd to allow per-sprite pivot positioning in addition to the pivot points on UISprite.
		// This is a normalized value, so .5 is the center.
		public Vector2 pivotOffset = new Vector2(.5f, .5f);
	...
	[HideInInspector][SerializeField] Coordinates mCoordinates = Coordinates.TexCoords;
	...
	// If we MUST use pixel coordinates, first enable it via this flag (else we'll throw exceptions)
	[HideInInspector] [NonSerialized] public bool allowPixelCoordinates = false;
	...
	public Material spriteMaterial
	{
		get
		{
			return (!System.Object.ReferenceEquals(mReplacement,null)) ? mReplacement.spriteMaterial : material;
		}
		set
		{
    ...
    private Dictionary<string, Sprite> mNameSpriteDataDic = null;

    
    void UpdateDic()
    {
        mNameSpriteDataDic = new Dictionary<string, Sprite>();
        for (int i = 0, imax = sprites.Count; i < imax; ++i)
        {
            Sprite s = sprites[i];
            if (!string.IsNullOrEmpty(s.name))
                mNameSpriteDataDic[s.name] = s;
        } 
    }
    	/// <summary>
	...
	// Changed for Alpha-Split support...
	...
	// Target texture isn't necessarily what is pointed at by the material; we may have derived RGB & Alpha textures.
	// So, we first check if there is an original "main" texture without the (RGB) suffix that should be the actual target.
	// Otherwise the atlas builder tries to modify the wrong target.
	...
	{ 
		get 
		{
			...
			{
				#if UNITY_EDITOR
				// If material points at : Assets/Data/HIR/NGUI/Atlases/Generic Uncompressed/Generic Uncompressed Hi (RGB).png
				// First check if we have: Assets/Data/HIR/NGUI/Atlases/Generic Uncompressed/Generic Uncompressed Hi.png
				if (material.mainTexture != null)
				{
					var path = UnityEditor.AssetDatabase.GetAssetPath( material.mainTexture );
					if (path.Contains( " (RGB)."))  //TODO: use constant from AlphaSplitInspector, which is currently in the editor assembly
					{
						path = path.Replace(" (RGB).", ".");
						var newAsset = UnityEditor.AssetDatabase.LoadMainAssetAtPath( path ) as Texture;
						if (newAsset != null)
						{
							return newAsset;
						}
	...
	public Coordinates coordinates
	{
		...
		set
		{
			...
			{
				// Zynga - actually, we can allow coordinate changes if no sprites have been defined yet
				if (sprites.Count > 0 && (material == null || material.mainTexture == null))
				{
					Debug.LogError("Can't switch coordinates until the atlas material has a valid material/texture: " + this.name);
					return;
				}

				// ZYGNA: We almost always want texcoord based coordinates, so we can change texture importer sizes..
				// For now, throw an error if a user tries pixel coordinates (unless it's explicitely allowed via allowPixelCoordinates) 
				if (value == Coordinates.Pixels && !allowPixelCoordinates)
				{
					throw new Exception("Must use texCoords for UIAtlas coordinates (not pixels); if an issue, contact KevinK or KrisG");
				}

				// assign the new coordinate system
				mCoordinates = value;

				// and convert any/all existing sprite coordinates
				for (int i = 0, imax = sprites.Count; i < imax; ++i)
				{
	...
	{
		if (!System.Object.ReferenceEquals(mReplacement,null))
		{
		...
		else if (!string.IsNullOrEmpty(name))
		{
			...
			*/
            Sprite s = null;
            mNameSpriteDataDic.TryGetValue(name, out s);
