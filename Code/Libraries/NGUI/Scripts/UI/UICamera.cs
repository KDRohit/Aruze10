...
public class UICamera : TICoroutineMonoBehaviour
{
	/// <summary>
	...
	public static Dictionary<Transform,Camera> sUICameraTransformDic = new Dictionary<Transform,Camera>();
			...
			if (mSel != value)
			{
				if (mSel != null)
				{
					...
					if (uicam != null)
					{
						...
						if (uicam.useKeyboard) Highlight(mSel, false);
						current = null;
					}
				}
				...
				if (mSel != null)
				{
					...
					if (uicam != null)
					{
						current = uicam;
						currentCamera = uicam.mCam;
						if (uicam.useKeyboard) Highlight(mSel, true);
	...
	// Enables input processing for this UICamera.
	public void enablePlatformInput()
	{
		// Special flag combinations that make input work on different platforms.
		// Note: all flags are listed in each case for readability, rather than making
		// someone reading this code refer back and forth to the default values.
#if UNITY_EDITOR
		useTouch = true;
		useMouse = true;
		useKeyboard = true;
		...
		useKeyboard = true;
		mIsEditor = false;
		Debug.LogError("Unknown platform for input in UICamera.");
#endif
	}
	
	// Disables input processing for this UICamera.
	public void disablePlatformInput()
	{
		useTouch = false;
		useMouse = false;
		useKeyboard = false;
	...
	void Awake ()
	{
		...
		sUICameraTransformDic[this.transform] = this.GetComponent<Camera>();
	...
	void OnDestroy () { mList.Remove(this); sUICameraTransformDic.Remove(this.transform); }
	...
	void Update ()
	{
		...
		bool didTouchNotify = false;	// Added by Zynga/Todd. Don't process mouse if touch event happened.

		// Process touch input
		if (useTouch)
		{
			didTouchNotify = ProcessTouches();
		}
		
		// Update mouse input
		// Zynga/Todd - Only call ProcessMouse() if no notifications were already sent in ProcessTouches().
		if (!didTouchNotify)
		{
	...
	public void ProcessMouse ()
	{
...
#if UNITY_WEBGL
		//we want to be able to do stuff with hover on WebGL that doesn't 
		...
		{
			// Zynga/Todd. If both mouse input and touch input are enabled,
			// then only use the first touch's position for the mouse position,
			// otherwise Unity averages multiple touches into a single position,
			// which is really weird because buttons between the touches get triggered
			// instead of something under one of the touch points.
			mMouse[0].pos = Input.GetTouch(0).position;
		}
		else
		{
		...
		if (updateRaycast)
		{
			// Zynga/Todd - Changed the first argument to use the assigned mouse position instead of Input.mousePosition,
			// so it will use the first touch position we assigned above if necessary.
			hoveredObject = Raycast(mMouse[0].pos, out lastHit) ? lastHit.collider.gameObject : fallThrough;
		...
		// Zynga/Todd: Added Input.touchCount == 0 condition to prevent mouse processing when touching. Mainly for Android.
	...
	public bool ProcessTouches ()
	{
		...
		for (int i = 0; i < Input.touchCount; ++i)
		{
			...
			didNotify = ProcessTouch(pressed, unpressed);
	...
	public bool ProcessTouch (bool pressed, bool unpressed)
	{
		// bool return value added by Zynga/Todd,
		// to prevent notifications for mouse if touch has already notified.
		bool didNotify = false;
		...
		if (pressed)
		{
			...
			// Zynga: Use the next line instead of the line after it. I don't know why we'd ever use "Always"
			// because it doesn't cancel click notification when the drag threshold has been met.
		...
		if (unpressed)
		{
			...
			if (currentTouch.pressed != null)
			{
					...
					currentTouch.totalDelta.magnitude < drag))
				{
					...
					{
						mSel = currentTouch.pressed;
						if (mSel != null && mSel.GetComponent<UIStateImageButton>() != null)
						{
							Notify(currentTouch.pressed, "OnSelect", true);
						}
					}
