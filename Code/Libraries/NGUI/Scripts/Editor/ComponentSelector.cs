...
public class ComponentSelector : ScriptableWizard
{
	...
	static public void Draw<T> (string buttonName, T obj, OnSelectionCallback cb, params GUILayoutOption[] options) where T : TICoroutineMonoBehaviour
	{
	...
	static public void Draw<T> (T obj, OnSelectionCallback cb, params GUILayoutOption[] options) where T : TICoroutineMonoBehaviour
	{
	...
	static void Show<T> (OnSelectionCallback cb) where T : TICoroutineMonoBehaviour
	{
	...
	bool DrawObject (MonoBehaviour mb)
	{
		...
		GUILayout.BeginHorizontal();
		{
			...
			string assetPath = AssetDatabase.GetAssetPath(mb.gameObject);
			string hierarchyPath = NGUITools.GetHierarchy(mb.gameObject);

			GUILayout.Label(hierarchyPath, "AS TextArea", GUILayout.Height(20f));
			GUILayout.Label(assetPath, "AS TextArea", GUILayout.Height(20f));
