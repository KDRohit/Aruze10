...
// Zynga KrisG: 
// NGUI UITexturePacker calls Texture2d.Resize, which cant handle compressed formats--sometimes this causes problems in atlas generation. 
// Unity's PackTextures doesnt have this problem.
// Also, we dont need to force-square atlases for iOS PVRTC. 
// The texture importer will automatically blow up non-square atlases to square for iOS PVRTC.
// Android will waste less tex memory by not forcing square.
// So for all these reasons, disable NGUI's AtlasPacker and force Unity's.
//#define FORCE_UNITY_ATLASPACKER
#define ALLOW_FORCE_SQUARE  
// ALLOW_FORCE_SQUARE is re-enabled, because the atlas prefab doesnt store the original texture dimensions, which
// means it pulls them from the texture-importer-PVRTC-resized rectangular texture and gets the sprite coordinates wrong.  
// TODO: store the original texture dimensions in the atlas
//
// disabled. Zynga KrisG: this wastes memory for Atlases that can fit in a rectangle.  iOS will be expanded to square by PVRTC importer automatically
//#define #if SET_MAINATLAS_FORMAT 

#pragma warning disable 0618 // Not using UnityEditor.AssetDatabase.IsOpenForEdit(string, StatusQueryOptions)
...
{
	static bool _generateAlphaSplitAtlases = true;  // Zynga KrisG: default to true
	...
	static void ReplaceSprites (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		// so we can restore the coordinate mode afterwards
		var originalCoordinateMode = atlas.coordinates;
		...
		// restore original coordinate mode, dont let our devs choose pixel mode
		atlas.coordinates = originalCoordinateMode;
	...
	static bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		else
		{
			...
			// Zynga KrisG: load from PNG directly so we dont run into problems with compressed formats
			tex = LoadPNG(oldPath);
		}
		...
		else
		{
			...
			EditorUtility.DisplayDialog("Operation Cancelled", "The selected sprites can't fit into the atlas.\n" +
	...
	void OnGUI ()
	{
		...
		if (create)
		{
				...
				NGUISettings.atlasName + " atlas with the textures currently selected in the Project View? All other sprites will be deleted.", "Yes", "No"))
			{
				...
				if (mat == null)
				{
					Shader shader = ShaderCache.find(NGUISettings.atlasPMA ? "Unlit/Premultiplied Colored" : "Unlit/Transparent Colored");
		...
		if (NGUISettings.atlasPadding > 1) { //Colors UI
		...
		GUILayout.Label((NGUISettings.atlasPadding == 1 ? "Pixel" : "Pixels") + " in-between sprites (default is 1)");
		GUI.color = Color.white;
		GUILayout.EndHorizontal();
		...
		{
			GUI.backgroundColor = Color.red;
			trimMessage = "This should not be checked unless updating an older atlas that requires it. Newer atlases shouldn't use this.";

		}
		else {
			GUI.backgroundColor = Color.white;
			trimMessage = "Remove any 100% alpha around the edges of sprites (default is OFF)";
		}

		NGUISettings.atlasTrimming = EditorGUILayout.Toggle("Trim Alpha", NGUISettings.atlasTrimming, GUILayout.Width(100f));

		//Changes the color of the trim alpha message label
		if (NGUISettings.atlasTrimming) { //Colors UI
			GUI.color = Color.red;
		}
		else {
			GUI.color = Color.white;
		...
		GUILayout.Label(trimMessage);

		//Resets the colors to white
		GUI.backgroundColor = Color.white;
		GUI.color = Color.white;
		GUILayout.EndHorizontal();

		/*
		//We don't use this setting ever, so I'm just commenting it out for now in case we ever have to turn it back on. - Paul S
		GUILayout.BeginHorizontal();
		...
		GUILayout.EndHorizontal();
		*/
		NGUISettings.atlasPMA = true; //Just set the value to true since we don't ever change this currently. - Paul S
		...
		GUILayout.Label("if off, use NGUI's atlas packer instead of Unity's");
		GUILayout.EndHorizontal();
		*/
		NGUISettings.unityPacking = false; //Just set the value to false since we don't ever change this currently. - Paul S

		#if ALLOW_FORCE_SQUARE
		if (!NGUISettings.unityPacking)
			{
			/*
			//We don't use this setting ever, so I'm just commenting it out for now in case we ever have to turn it back on. - Paul S
			...
			GUILayout.EndHorizontal();
			*/
			NGUISettings.forceSquareAtlas = true; //Just set the value to true since we don't ever change this currently. - Paul S
		}
		#endif
	#endif
		...
		//We don't use this setting ever, so I'm just commenting it out for now in case we ever have to turn it back on. - Paul S
		GUILayout.BeginHorizontal();
		NGUISettings.allow4096 = false;
		EditorGUILayout.Toggle("4096x4096", NGUISettings.allow4096, GUILayout.Width(100f));
		GUILayout.Label("if off, limit atlases to 2048x2048");
		GUILayout.EndHorizontal();
		*/
		NGUISettings.allow4096 = false; //Just set the value to true since we don't ever change this currently. - Paul S

		GUILayout.BeginHorizontal();
		/*
		if (_generateAlphaSplitAtlases) { //Colors UI
		...
		*/
		...
		_generateAlphaSplitAtlases = EditorGUILayout.Toggle("Alpha-Split", ((PlayerPrefs.GetInt("alphaSplit") == 1) ? true : false), GUILayout.Width(100f));

		if (_generateAlphaSplitAtlases)	{ //Sets playerPrefs to 0 (false) or 1 (true)
			PlayerPrefs.SetInt("alphaSplit", 1);
			//GUI.color = Color.yellow;
		}
		else {
			PlayerPrefs.SetInt("alphaSplit", 0);
			//GUI.color = Color.white;
		} 

		GUILayout.Label("If on, additionally generate alpha-only atlases for optimized Android compression on old Android devices");
		GUILayout.EndHorizontal();

		//Reset colors for UI beneath atlas split stuff
		GUI.backgroundColor = Color.white;
		GUI.color = Color.white;
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				foreach (KeyValuePair<string, int> iter in spriteList)
				{
					...
					//colors the full text string cyan if a user has a texture selected to "Update"
					if (iter.Value == 1) { //iter 1 == update
						GUI.color = Color.cyan;
					}
					else if (iter.Value == 2)	{ //iter 2 == add
					...
					//colors the full text string red if the sprite is marked for delete (the "X" button is pressed)
					if (mDelNames.Contains(iter.Key))
					{
						GUI.color = Color.red;
					}

					GUILayout.Label(index.ToString(), GUILayout.Width(26f)); //the labels for the sprite #s

					/*
					//Commenting this out because the "OL TextField" GUI style was changing all the sprite name text to red, causing some weird color interactions - Paul S.
					if (GUILayout.Button(iter.Key, "OL TextField", GUILayout.Height(20f)))
					selection = iter.Key;
					*/

					GUILayout.Label(iter.Key, GUILayout.Height(20f)); //The labels for each of the sprite names
					GUI.color = Color.white; //sets UI back to white from the red delete color change above
					float digit_spacing = 70f;
					...
					{
						GUI.color = Color.cyan;
						GUILayout.Label(("Update " + index.ToString()), GUILayout.Width(digit_spacing));
						GUI.color = Color.white;
					}
					else
					{
						if (mDelNames.Contains(iter.Key))
						{
							...
							if (GUILayout.Button(("Delete " + index.ToString()), GUILayout.Width(digit_spacing + 5)))
							{
								delete = true;
							}
							GUI.backgroundColor = Color.green;
				...
				bAtlasChanged |= (delete || replace || update);
				...
				if (delete)
				{
					...
					UpdateAtlas(textures, keepSprites:true);
				else if (replace)
					UpdateAtlas(textures, keepSprites:false);

				if (bAtlasChanged)
				{
					DoneWithAtlasUpdates(create);
				}
	...
	// texture must be set to 'readable' to manipulate it in editor.  then after we are done,
	// must set it to unreadable so it is properly optimized when loaded in-game.
	public static void SetTextureSettings(Texture2D texture, bool isReadable, bool setFinalFormat)
	{
		...
		#if SET_MAINATLAS_FORMAT
			// slightly ugly, throw in a change back to PVRTC4 for iOS right before we do final reimport
			if (setFinalFormat)
			{
				//tImporter.textureFormat = TextureImporterFormat.AutomaticCompressed;  // should be safe now that AlphaTextureGenerator doesnt ever read the data from the imported texture

				// cannot be compressed right now because atlas builder will not build atlas properly
				// from rectangular atlases on ios, due to importer of PVRTC expanding the texture size to square, which
				// doesnt match the original size.

				//Debug.LogWarningFormat("Setting Final Format for main atlas texture {0} before reimporting", texture.name);
				// TODO: add other platforms here?  I'm changing the default below as well.
				//tImporter.textureFormat = TextureImporterFormat.AutomaticCompressed;

				CommonEditor.setTextureImporterOverrides(tImporter, "iPhone", AlphaSplitTextureInspector.MAX_TEXTURE_SIZE,
													 AlphaSplitTextureInspector.IPHONE_RGB_TEX_FORMAT,  // Note: I want PVRTC4 here, not RGBA  
													 AlphaSplitTextureInspector.IPHONE_MAIN_TEX_QUALITY, false);			

				// leaving compression on ETC1 because Kevin wants to explore getting rid of the 'RGB' texture and just using the main texture for
				// both Android and iOS, since the RGB texture is just a copy of main one.
			#if true
				// AlphaSplitTextureInspector.setAlphaSplitEnabled(atlasTex, enabled:true) in DoneWithAtlasUpdates breaks (returns false) if texture is set to
				// ETC1 because it has no alpha, so the case where you create an atlas without alpha-split, it ends up as ETC1, and then later you want to enable
				// it is broken, so revert to RGBA for now, until we want to get rid of RGB atlas and replace it with main atlas to reduce asset redundancy (those
				// files are copies of each other)
				CommonEditor.setTextureImporterOverrides(tImporter, "Android", AlphaSplitTextureInspector.MAX_TEXTURE_SIZE, 
													 AlphaSplitTextureInspector.ANDROID_MAIN_TEX_FORMAT,  // TODO: I want ETC1 (ANDROID_RGB_TEX_FORMAT) here, not RGBA, but it breaks 
					                                 AlphaSplitTextureInspector.ANDROID_MAIN_TEX_QUALITY, false);
			#else
				CommonEditor.setTextureImporterOverrides(tImporter, "Android", AlphaSplitTextureInspector.MAX_TEXTURE_SIZE,
					TextureImporterFormat.RGBA32,  // do this if you're not sure AlphaTextureGenerator is reading uncompressed textures
					AlphaSplitTextureInspector.ANDROID_MAIN_TEX_QUALITY, false);
			#endif
			}
		#else
			// for now, the main atlas will always be RGBA, and the RGB texture will be the only one with compression settings
			// because ios rectangular textures cause problems when we try to read it (comes through importer as square texture, but png is rectangle)
			tImporter.ClearPlatformTextureSettings("iPhone");
			tImporter.ClearPlatformTextureSettings("Android");
		#endif
	...
	// Last thing we do after updating atlases
	static void DoneWithAtlasUpdates(bool bCreatingNewAtlas)
	{
		// Notify any dependent assets/tools (such as texture alpha-splitter) that our atlas has been updated.
		// For now, an AssetRefresh is sufficient.
		//Debug.LogWarningFormat("calling AssetDatabase.Refresh() to notify texture alpha-splitter");
		AssetDatabase.Refresh();

		if ( _generateAlphaSplitAtlases)
		{
			//Debug.LogWarningFormat("setting alpha-split enabled on new atlas");
			// this requires AssetDatabase to be up-to-date
			Texture2D atlasTex = NGUISettings.atlas.texture as Texture2D;

			if (bCreatingNewAtlas || !AlphaSplitTextureInspector.isTextureAlphaSplitEnabled(AssetDatabase.GetAssetPath(atlasTex)))
			{
				AlphaSplitTextureInspector.setAlphaSplitEnabled(atlasTex, enableAlphaSplit:true);
			}

			// make texture readable so we can read its PNG contents in editor to Generate the alpha-split textures
			SetTextureSettings(atlasTex, isReadable:true, setFinalFormat:false);

			AlphaTextureGenerator.GenerateAlphaSplitTextures(atlasTex);

			// make unreadable so it is optimized at runtime (ie. doesnt use 'system' memory on certain devices)
			SetTextureSettings(atlasTex, isReadable:false, setFinalFormat:true);
		}

		AssetDatabase.SaveAssets(); // make sure updated atlas prefabs and materials are immediately written to disk, dont assume user will remember to hit 'Save Project'
	}

	// Utility function to load a Texture2D from a PNG file
	static Texture2D LoadPNG(string filePath) 
	{
		...
		if (System.IO.File.Exists(filePath))
		{
			...
			tex = new Texture2D(2, 2);  // junk initial size
