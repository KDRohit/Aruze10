...
public class UIAtlasInspector : TICoroutineMonoBehaviourEditor
{
	...
	// Zynga/Todd added:
	private Vector2int originalTexSize = Vector2int.zero;
	
	// Since we may want to use non-square atlas textures for optimal performance on Android,
	// but PVRTC requires square on iOS, we might be stretching a non-square texture to square
	// when importing for iOS. Because of that, we must get the original native texture size
	// instead of the imported texture size.
	private void refreshOriginalTexSize()
	{
		UIAtlas atlas = target as UIAtlas;
		if (atlas == null || atlas.spriteMaterial == null || atlas.spriteMaterial.mainTexture == null)
		{
			originalTexSize.x = originalTexSize.y = 0;
			return;
		}
		originalTexSize = CommonEditor.getOriginalTextureSize(atlas.spriteMaterial.mainTexture);
	}

	void OnEnable () { instance = this; refreshOriginalTexSize(); }
				...
				if (!mConfirmDelete && sprite != null)
				{
					...
					if (tex != null)
					{
						...
						if (mAtlas.coordinates == UIAtlas.Coordinates.Pixels)
						{
							...
							border = NGUIEditorTools.IntPadding("Slicing Pixels", border);
							...
							// This isn't useful for directly editing, so omit showing it to have a cleaner inspector.
							// Not that we're allowing pixel mode to be used anymore anyway, but still.
							// GUI.backgroundColor = green;
							// NGUIEditorTools.IntRect("Dimensions", sprite.outer);

							GUI.backgroundColor = Color.white;
						}
						else
						{
							// Added by Zynga/Todd - Show a pixel-based Border input for
							// specifying slicing data even when in TexCoord mode.
							Rect pixelInner = NGUIMath.ConvertToPixels(sprite.inner, originalTexSize.x, originalTexSize.y, true);
							Rect pixelOuter = NGUIMath.ConvertToPixels(sprite.outer, originalTexSize.x, originalTexSize.y, true);
							
							Vector4 border = new Vector4(
								pixelInner.xMin - pixelOuter.xMin,
								pixelInner.yMin - pixelOuter.yMin,
								pixelOuter.xMax - pixelInner.xMax,
								pixelOuter.yMax - pixelInner.yMax);

							GUI.backgroundColor = blue;
							border = NGUIEditorTools.IntPadding("Slicing Pixels", border);
							GUI.backgroundColor = Color.white;
							
							// Calculate the inner rect in pixels.
							inner.xMin = pixelOuter.xMin + border.x;
							inner.yMin = pixelOuter.yMin + border.y;
							inner.xMax = pixelOuter.xMax - border.z;
							inner.yMax = pixelOuter.yMax - border.w;

							// Now convert the inner rect back to TexCoords before saving the values.
							inner = NGUIMath.ConvertToTexCoords(inner, originalTexSize.x, originalTexSize.y);
							// End of Zynga/Todd

							// Draw the inner and outer rectangle dimensions
							// Showing this isn't useful since it can't be edited effectively. Just disable it.
							// GUI.backgroundColor = green;
							// EditorGUILayout.RectField("Outer Rect", sprite.outer);
							// GUI.backgroundColor = blue;
							// EditorGUILayout.RectField("Inner Rect", sprite.inner);
							// GUI.backgroundColor = Color.white;
						}
						...
						// Added by Zynga/Todd.
						sprite.pivotOffset = NGUIEditorTools.FloatPair("Pivot Offset", "Left", "Top", sprite.pivotOffset);
						sprite.pivotOffset.x = Mathf.Clamp01(sprite.pivotOffset.x);
						sprite.pivotOffset.y = Mathf.Clamp01(sprite.pivotOffset.y);
						mAtlas.MarkAsDirty();
					}
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
		...
		// Zynga/Todd: added sprite.pivotOffset argument.
		NGUIEditorTools.DrawSprite(tex, rect, outer, inner, uv, Color.white, null, sprite.pivotOffset);
	}
}
