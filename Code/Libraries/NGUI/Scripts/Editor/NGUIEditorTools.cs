...
public class NGUIEditorTools
{
	...
	static bool MakeTextureReadable (string path, bool force)
	{
		...
		// MODIFIED FOR TEXTURE IMPORT SETTINGS FOR HIR-898 by Adam Malloy
		// Took out a few setting changes that didn't need to be here anymore.
		if (force ||
	...
	static bool MakeTextureAnAtlas (string path, bool force)
	{
			...
			settings.npotScale != TextureImporterNPOTScale.ToNearest)
		{
			...
			settings.aniso = 0;   //Zynga: we're a 2D game, don't need any anisotropic filtering; we default this to 0 everywhere -KK
			settings.wrapMode = TextureWrapMode.Clamp;
			settings.npotScale = TextureImporterNPOTScale.ToNearest;
	...
	static public Texture2D ImportTexture (string path, bool forInput, bool force)
	{
		if (!string.IsNullOrEmpty(path))
		{
			...
			//Debug.LogWarningFormat("Importing path {0}",path);
	...
	static public Vector2 FloatPair (string prefix, string leftCaption, string rightCaption, Vector2 vec)
	{
		GUILayout.BeginHorizontal();
		...
		Vector2 retVal;
		retVal.x = EditorGUILayout.FloatField(leftCaption, vec.x, GUILayout.MinWidth(30f));
		retVal.y = EditorGUILayout.FloatField(rightCaption, vec.y, GUILayout.MinWidth(30f));
	...
	static public void RegisterUndo (string name, params Object[] objects)
	{
		...
		else
		{
			...
			{
				//Is Scene root object
				if (go.transform.parent == null)
				{
					roots.Add (go);
				}
			}
			Undo.RecordObjects(roots.ToArray(), name);
			///NGUI Warning Fix End
		}
	}
	...
	/// </summary>
	///NGUI Warning Fix Start
	static public void RegisterCreatedObjectUndo (string name, Object obj)
	{
	...
	{
		DrawSprite(tex, rect, outer, inner, uv, color, null, new Vector2(.5f, .5f));
	}

	/// <summary>
	/// Draw the specified sprite. Overload added by Zynga/Todd.
	/// </summary>

	public static void DrawSprite (Texture2D tex, Rect rect, Rect outer, Rect inner, Rect uv, Color color, Material mat)
	{
		DrawSprite(tex, rect, outer, inner, uv, color, mat, new Vector2(.5f, .5f));
	}
	...
	public static void DrawSprite (Texture2D tex, Rect rect, Rect outer, Rect inner, Rect uv, Color color, Material mat, Vector2 pivotOffset)
	{
		...
		GUI.BeginGroup(outerRect);
		{
			...
			// Zynga/Todd: Draw the pivot offset as a white crosshair.
			int centerX = (int)(pivotOffset.x * outerRect.width);
			int centerY = (int)(pivotOffset.y * outerRect.height);
			NGUIEditorTools.DrawTiledTexture(new Rect(centerX - 10, centerY, 20, 1f), whiteTexture);
			NGUIEditorTools.DrawTiledTexture(new Rect(centerX, centerY - 10, 1, 20), whiteTexture);
		}
		GUI.EndGroup();
	...
	// START BLOCK ADDED FOR PLATFORM SPECIFIC TEXTURE IMPORT SETTINGS FOR HIR-2609 by Adam Malloy
	public static PlatformTextureSettings GetPlatformTextureSettings(Texture texture, string platform)
	{
		string path = AssetDatabase.GetAssetPath(texture.GetInstanceID());
		return GetPlatformTextureSettings(path, platform);
	}
	public static PlatformTextureSettings GetPlatformTextureSettings(string path, string platform)
	{
		if (string.IsNullOrEmpty(path)) return null;
		TextureImporter ti = AssetImporter.GetAtPath(path) as TextureImporter;
		if (ti == null) return null;

		PlatformTextureSettings settings = new PlatformTextureSettings(platform);
		ti.GetPlatformTextureSettings(settings.platform, out settings.maxTextureSize, out settings.textureFormat, out settings.compressionQuality);

		return settings;
	}


	// END BLOCK ADDED FOR PLATFORM SPECIFIC TEXTURE IMPORT SETTINGS FOR HIR-2609 by Adam Malloy
}
