...
public class NGUISettings
{
	...
	static bool mShowSpriteNames = true;	// Zynga / Gillissie
	static string mFontName = "New Font";
	static string mAtlasName = "New Atlas";
	static string mSpriteName;
	//static int mAtlasPadding = 1; 	//Telos / Mike Wood - HiR padding default.
	static int mAtlasPadding = 4;
	//static public bool mAtlasTrimming = true; 	//Telos / Mike Wood - HiR aslpha trim default
	static public bool mAtlasTrimming = false;
	static public bool mAtlasPMA = false;
	//static bool mUnityPacking = true;	//Telos / Mike Wood - HiR packing default
	static bool mUnityPacking = false;
	static bool mForceSquare = true;  // TODO: Zynga KrisG: this wastes memory for Atlases that can fit in a rectangle.  iOS will be expanded to square by PVRTC importer automatically
	...
	static void Load ()
	{
		...
		mShowSpriteNames= EditorPrefs.GetBool("NGUI Show Sprite Names", false);	// Zynga / Gillissie.
	...
	static void Save ()
	{
		...
		EditorPrefs.SetBool("NGUI Show Sprite Names", mShowSpriteNames);	// Zynga / Gillissie.
	...
	/// <summary>
	/// Added by Zynga / Gillissie.
	/// Whether to show sprite names on the sprite selector wizard.
	/// </summary>

	static public bool showSpriteNames
	{
		get
		{
			if (!mLoaded) Load();
			return mShowSpriteNames;
		}
		set
		{
			if (mShowSpriteNames != value)
			{
				mShowSpriteNames = value;
				EditorPrefs.SetBool("NGUI Show Sprite Names", mShowSpriteNames);
			}
		}
	}
