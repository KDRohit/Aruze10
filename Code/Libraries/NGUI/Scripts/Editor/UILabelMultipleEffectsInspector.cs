...
[CustomEditor(typeof(UILabelMultipleEffects))]
public class UILabelMultipleEffectsInspector : UIWidgetInspector
{
	UILabelMultipleEffects mLabel;
	...
	public override void OnSceneGUI()
	{
		...
		Vector3[] corners = NGUIMath.CalculateWidgetCorners(mWidget, (mWidget as UILabelMultipleEffects).maxSize);
	...
	{
		mLabel = mWidget as UILabelMultipleEffects;
		...
		if (mLabel.font != null)
		{
			...
			/// effect 1
			/// 
			UILabelMultipleEffects.Effect effect;
			Color c;
			Vector2 offset;

			GUILayout.BeginHorizontal();
			{
				effect = (UILabelMultipleEffects.Effect)EditorGUILayout.EnumPopup("Effect", mLabel.effectStyle, GUILayout.Width(170f));
				if (effect != mLabel.effectStyle) { RegisterUndo(); mLabel.effectStyle = effect; }

				if (effect != UILabelMultipleEffects.Effect.None)
				{
					c = EditorGUILayout.ColorField(mLabel.effectColor);
					if (mLabel.effectColor != c) { RegisterUndo(); mLabel.effectColor = c; }
				}
			}
			GUILayout.EndHorizontal();

			if (mLabel.effectStyle != UILabelMultipleEffects.Effect.None)
			{
				GUILayout.Label("Distance", GUILayout.Width(70f));
				GUILayout.BeginHorizontal();
				offset = EditorGUILayout.Vector2Field("", mLabel.effectDistance);
				GUILayout.Space(20f);
			...
			GUILayout.BeginHorizontal();
			{
				effect = (UILabelMultipleEffects.Effect)EditorGUILayout.EnumPopup("Effect", mLabel.effectStyle2, GUILayout.Width(170f));
				if (effect != mLabel.effectStyle2) { RegisterUndo(); mLabel.effectStyle2 = effect; }

				if (effect != UILabelMultipleEffects.Effect.None)
				{
					c = EditorGUILayout.ColorField(mLabel.effectColor2);
					if (mLabel.effectColor2 != c) { RegisterUndo(); mLabel.effectColor2 = c; }
				}
			}
			GUILayout.EndHorizontal();

			if (mLabel.effectStyle2 != UILabelMultipleEffects.Effect.None)
			{
				GUILayout.Label("Distance", GUILayout.Width(70f));
				GUILayout.BeginHorizontal();
				offset = EditorGUILayout.Vector2Field("", mLabel.effectDistance2);
				GUILayout.Space(20f);

				if (offset != mLabel.effectDistance2)
				{
					RegisterUndo();
					mLabel.effectDistance2 = offset;
				}
				GUILayout.EndHorizontal();
			}

			GUILayout.BeginHorizontal();
			UILabelMultipleEffects.ColorMode colorMode = (UILabelMultipleEffects.ColorMode)EditorGUILayout.EnumPopup("Color Mode", mLabel.colorMode, GUILayout.Width(170f));

			if (colorMode != mLabel.colorMode) { RegisterUndo(); mLabel.colorMode = colorMode; }
			if (colorMode == UILabelMultipleEffects.ColorMode.Gradient)
			{
