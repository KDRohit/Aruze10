...
static public class NGUIMath
{
	...
	/// </summary>
	//Added by Zynga - Mike Wood (Telos) - added ability to pass in size, this allow for reuse when calculating max and actual size of a label.
	static public Vector3[] CalculateWidgetCorners (UIWidget w)
	{
		return CalculateWidgetCorners(w, w.relativeSize);
	}
	static public Vector3[] CalculateWidgetCorners (UIWidget w, Vector2 size)
	{
	...
	/// </summary>
#if USE_UNSAFE
	static unsafe public Bounds CalculateAbsoluteWidgetBounds (Transform trans)
	{
		if (trans != null)
		{
            ...
            float vmax_x = float.MinValue;
            float vmax_y = float.MinValue;
            float vmax_z = float.MinValue;
            float vmin_x = float.MaxValue;
            float vmin_y = float.MaxValue;
            float vmin_z = float.MaxValue;
			...
			for (int i = 0, imax = widgets.Length; i < imax; ++i)
			{
                ...
                fixed(float *ptr_ = &corners[0].x)
                {
                    float *ptr = ptr_;
                    
                    //corners[0]
                    if(*ptr > vmax_x) vmax_x = *ptr;       
                    if(*ptr < vmin_x) vmin_x = *ptr;   
                    ptr++;
                    if(*ptr > vmax_y) vmax_y = *ptr;       
                    if(*ptr < vmin_y) vmin_y = *ptr;   
                    ptr++;
                    if(*ptr > vmax_z) vmax_z = *ptr;       
                    if(*ptr < vmin_z) vmin_z = *ptr;   
                    ptr++;
                    //corners[1]
                    if(*ptr > vmax_x) vmax_x = *ptr;       
                    if(*ptr < vmin_x) vmin_x = *ptr;   
	...
	/// </summary>
#if USE_UNSAFE   
	static unsafe public Bounds CalculateRelativeWidgetBounds (Transform relativeTo, Transform content, bool considerInactive)
	{
		if (content != null)
		{
			...
			if (widgets.Length > 0)
			{
				...
				for (int i = 0, imax = widgets.Length; i < imax; ++i)
				{
					...
					Vector3[] corners = w.GetSidesRelativeTo(ref toLocal);
