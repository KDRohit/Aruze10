...
/*
Tool to help re-link animation objects if something is renamed or moved in the hierarchy,
since Unity amazingly doens't do this automatically.

Found at http://answers.unity3d.com/questions/662382/how-to-change-objects-hierarchy-level-in-animation.html
*/
...
public class AnimationHierarchyEditor : EditorWindow {
	...
	private string sOriginalRoot = "";
	private string sNewRoot = "";
	...
	void OnSelectionChange() {
		if (Selection.objects.Length > 1 )
		{
			...
			animatorObject = FindMatchingAnimator((AnimationClip)Selection.activeObject);
			FillModel();
		} else {
			animationClips.Clear();
		}
	...
	void OnGUI() {
		...
		if (animationClips.Count > 0 ) {
			...
			EditorGUILayout.BeginHorizontal();
			GUILayout.Label("Find:", GUILayout.Width(columnWidth));
			GUILayout.Label("Replace:", GUILayout.Width(columnWidth));
			EditorGUILayout.EndHorizontal();
			...
			EditorGUILayout.BeginHorizontal();
			if (GUILayout.Button("Replace Root", GUILayout.Width(columnWidth * 2))) {
			...
			GUILayout.Label("Animated Properties:");
			EditorGUILayout.EndHorizontal();
	...
	void GUICreatePathItem(string path) {
		...
		EditorGUILayout.LabelField(
			properties != null ? properties.Count.ToString() : "0", GUILayout.Width(60)
		);
		...
		for ( int iCurrentClip = 0; iCurrentClip < animationClips.Count; iCurrentClip++ )
		{
			...
			for ( int iCurrentPath = 0; iCurrentPath < pathsKeys.Count; iCurrentPath ++)
			{
				...
				for (int i = 0; i < curves.Count; i++) 
				{
					...
					{
						// Commenting this out because it prevents replacing with empty
						//
	...
	Animator FindMatchingAnimator(AnimationClip selectedClip)
	{
		string selectedClipName = selectedClip.name;
		...
		foreach (GameObject go in Resources.FindObjectsOfTypeAll(typeof(GameObject)) as GameObject[])
		{
			...
			foundAnimator = go.GetComponent<Animator>();
			if (foundAnimator == null || foundAnimator.runtimeAnimatorController == null)
			{
				continue;
			}
			string foundAnimatorName = foundAnimator.runtimeAnimatorController.name;

			if(selectedClipName == foundAnimatorName || selectedClipName + "_controller" == foundAnimatorName)
			{
				break;
			}
		}
