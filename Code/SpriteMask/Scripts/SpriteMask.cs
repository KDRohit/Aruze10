...
public class SpriteMask : SpriteMaskingComponent
{
	...
	private bool isRendererSet = false;
	...
	public bool inverted {
		...
		set {
			if (_inverted != value) {
				...
				updateSprites (isCleaningUp:false);
			}
		}
	}
	...
	private Material defaultSpriteMaterial {
		get {
			if (_defaultSpriteMaterial == null) {
				Shader shader = ShaderCache.find (_defaultSpriteShaderName);
	...
	private Material maskMaterial {
		get {
			if (_maskMaterial == null) {
				Shader shader = ShaderCache.find (SHADER_MASK_ROOT);
...
//		}
	...
	void OnDisable ()
	{
		...
		if (gameObject.activeInHierarchy) {
			...
			updateSprites (isCleaningUp:false);
			updateMaskingComponents (isCleaningUp:false);
		}
	...
	////////////////////////// ZYNGA ADDED FUNCTIONALITY /////////////////////////
	// Creating Zynga specific handling for OnDestroy to make sure that shared materials
	// aren't left in a broken state after a SpriteMask is destroyed while the game isn't running
	void OnDestroy()
	{
		if (!Application.isPlaying)
		{
			// Go through any material that is touched by this mask and reset the masking values to default values
			updateSprites(isCleaningUp:true);
			updateMaskingComponents(isCleaningUp:true);
		}
	}

	// Function to add to masked object list so we have better control at who are adding objects to the list
	public void addMaskedObject(Transform addedObj)
	{
		maskedObjects.Add(addedObj);
		update();
	}

	public void removeMaskedObject(Transform objectToRemove)
	{
		maskedObjects.Remove(objectToRemove);
		update();
	}
	////////////////////////// END ZYNGA FUNCTIONALITY /////////////////////////////
...
////////////////////////// ZYNGA ADDED FUNCTIONALITY /////////////////////////
		...
		// When symbols get cached, they gain an instanced material
		// This instanced material needs to be updated to match the maskMaterial held in this script
		// @note : (Scott) isUsingDuplicatedSharedMaterials should be able to be used to fix the issue where
		// preview symbols made while the game isn't running all share the same material values causing only
		// one of the symbols to render correctly.  However it seems that overriding the shared materials
		// can also cause some strange issues, so I think we may need to restore the original set when
		// the preview symbols are disabled.  Once that issue is fixed we should be able to enable
		// isUsingDuplicatedSharedMaterials (and consider stripping out the old code)
		copyMaskedMaterialSettingsToAllSpriteMaskingComponents(m, isUsingDuplicatedSharedMaterials:false);
...
////////////////////////// ZYNGA ADDED FUNCTIONALITY /////////////////////////
	// When symbols get cached, they gain an instanced material
	// This instanced material needs to be updated to match the maskMaterial held in this script
	private void copyMaskedMaterialSettingsToAllSpriteMaskingComponents(Material passedMaskedMaterial, bool isUsingDuplicatedSharedMaterials)
	{
		// Start with this object
		SpriteMaskingComponent maskParent = getMaskingComponent(transform);
		Renderer maskRenderer = maskParent.owner.getRenderer();
		if (maskRenderer != null)
		{
			copyMaskedMaterialSettingsToSpriteMaskingComponent(passedMaskedMaterial, maskRenderer, isUsingDuplicatedSharedMaterials);
		}

		// Now handle any masked objects outside the hierarchy of this SpriteMask
		if (maskedObjects != null)
		{
			for (int i = 0; i < maskedObjects.Count; i++)
			{
				if (maskedObjects[i] == null || maskedObjects[i].gameObject == null)
				{
					continue;
				}
				copyRecursiveChildMasking(passedMaskedMaterial, maskedObjects[i], isUsingDuplicatedSharedMaterials);
			}
		}
	}

	private void copyRecursiveChildMasking(Material passedMaskedMaterial, Transform maskingTransform, bool isUsingDuplicatedSharedMaterials)
	{
		Renderer passedRenderer = getRenderer(maskingTransform);
		if (passedRenderer != null)
		{
			copyMaskedMaterialSettingsToSpriteMaskingComponent(passedMaskedMaterial, passedRenderer, isUsingDuplicatedSharedMaterials);
		}
		int childsCount = maskingTransform.childCount;
		if (childsCount > 0)
		{
			for (int i = 0; i < childsCount; i++)
			{
				copyRecursiveChildMasking(passedMaskedMaterial, maskingTransform.GetChild(i), isUsingDuplicatedSharedMaterials);
			}
		}
		
	}

	private void copyMaskedMaterialSettingsToSpriteMaskingComponent(Material passedMaskedMaterial, Renderer passedRenderer, bool isUsingDuplicatedSharedMaterials)
	{
		if (passedRenderer != null)
		{
			if (Application.isPlaying)
			{
				Material[] ownerMaterials = passedRenderer.materials;

				for (int i = 0; i < ownerMaterials.Length; i++)
				{
					Material attachedMaterial = ownerMaterials[i];
					updateMaterial(attachedMaterial, 
						passedMaskedMaterial.GetInt(propertyStencil), 
						(CompareFunction)passedMaskedMaterial.GetInt(propertyStencilComp), 
						passedMaskedMaterial.GetInt(propertyStencilReadMask));				
				}
			}
			else
			{
				Material[] ownerSharedMaterials = passedRenderer.sharedMaterials;
				Material[] modifiedSharedMaterials = new Material[ownerSharedMaterials.Length];

				for (int i = 0; i < ownerSharedMaterials.Length; i++)
				{
					if (ownerSharedMaterials[i] != null)
					{
						if (isUsingDuplicatedSharedMaterials)
						{
							Material duplicateMaterial = new Material(ownerSharedMaterials[i]);
							updateMaterial(duplicateMaterial, 
								passedMaskedMaterial.GetInt(propertyStencil), 
								(CompareFunction)passedMaskedMaterial.GetInt(propertyStencilComp), 
								passedMaskedMaterial.GetInt(propertyStencilReadMask));
							modifiedSharedMaterials[i] = duplicateMaterial;
						}
						else
						{
							updateMaterial(ownerSharedMaterials[i], 
								passedMaskedMaterial.GetInt(propertyStencil), 
								(CompareFunction)passedMaskedMaterial.GetInt(propertyStencilComp), 
								passedMaskedMaterial.GetInt(propertyStencilReadMask));
						}
				...
				if (isUsingDuplicatedSharedMaterials)
				{
					passedRenderer.sharedMaterials = modifiedSharedMaterials;
				}
			}
		}
		else
		{
			Debug.LogError("SpriteMask.copyMaskedMaterialSettingsToSpriteMaskingComponent() - meshRenderer was NULL!");
		}
	}
////////////////////////// END ZYNGA FUNCTIONALITY /////////////////////////////
	...
	/// </summary>
	public void updateMaskingComponents (bool isCleaningUp)
	{
		...
		for (int i = 0; i < this.maskedObjects.Count; i++) {
			...
			if (masking != this) {
				...
				updateSprites (maskedObjects [i], isCleaningUp);
			} else {
	...
	public void updateSprites (bool isCleaningUp)
	{
		updateSprites (transform, isCleaningUp);
	}

	/// <summary>
	/// Updates sprite that is on the 'trans' and all sprites that are "under" (are child of) the 'trans' object.
	/// </summary>
	public void updateSprites (Transform trans, bool isCleaningUp)
	{
#if SPRITE_MASK_DEBUG
		log ("updateSprites >>>");
#endif
	...
	private void destroyMeshComponents ()
	{
		...
		//Do not destroy the maskRenderer if its of type SpriteRenderer
		//so we do not lose the cutom spriterenderer settings
		if (maskRenderer != null && !(maskRenderer is SpriteRenderer)) {
#if UNITY_EDITOR
			if (!Application.isPlaying)
				DestroyImmediate (maskRenderer);
			else
#endif
