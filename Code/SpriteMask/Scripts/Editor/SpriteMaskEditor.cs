...
public class SpriteMaskEditor : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		PrefabInstanceStatus instanceStatus = PrefabUtility.GetPrefabInstanceStatus(mask);
		PrefabAssetType assetType = PrefabUtility.GetPrefabAssetType(mask);
		// This API is confusing but this seems to be the only way to figure this out now.
		bool isPrefab = assetType != PrefabAssetType.NotAPrefab && instanceStatus == PrefabInstanceStatus.NotAPrefab;
		...
		// @zynga_note : (Scott Lepthien) - Removing this option from being shown, since it is experimental
		// and I'm not sure if it will cause issues with the normal way we've been using SpriteMask so far.
		...
		if (foldoutAdvance) {
			...
			if (newIdx != idx) {
				switch (newIdx) {
				case 0:
					newShader = ShaderCache.find (SpriteMask.SHADER_SPRITE_DEFAULT);
					break;
				case 1:
					newShader = ShaderCache.find (SpriteMask.SHADER_SPRITE_DIFFUSE);
					break;
				}
			}
